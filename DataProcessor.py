...
def redFlagsS1(sheetName):
    ...
    for i in performanceValues:
        ...
        if i == 5 and float(removeNonNumeric(row[i].value, additionalSet)) < -350: #PE Under -350
            return errorHandler(errorNum + 1, sheetName)
        if i == 5 and float(removeNonNumeric(row[i].value, additionalSet)) > 350: #PE Over 350
...
def getErrorCode(input):
    switch = {
        ...
        1: "E1: P/E Ratio is below the cutoff of -350",
